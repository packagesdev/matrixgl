...
void initSettings(sceneSettings * inSettings)
{
...
void draw_text1(sceneSettings * inSettings)
{
   float x,y;
   long p=0,b=0;
   int c,c_pic;
...
void draw_text2(int mode,sceneSettings * inSettings)
{
   float x,y;
   long p=0;
   for(y=_text_y;y>-_text_y;y--)
...
int scrollScene(int mode,sceneSettings * inSettings)
{
	int a, s=0;
...
int make_change(sceneSettings * inSettings)
{
   ...
   if(r<inSettings->text_x && inSettings->text_light[r]!=0) inSettings->text_light[r]=255; /* white nodes */
	...
	if(inSettings->mode2) return scrollScene(inSettings->mode2,inSettings);
    
    return 1;
}


int cbRenderScene(sceneSettings * inSettings)
{  
   int tResult;
...
void ourInit(int Width,int Height,sceneSettings * inSettings) 
{
   ...
   inSettings->speed = (unsigned char *)tmalloc(inSettings->text_x);
   inSettings->text= (unsigned char *)tmalloc(inSettings->text_x*(text_y+1));
   inSettings->text_light = (unsigned char *)tmalloc(inSettings->text_x*(text_y+1));
   inSettings->bump_pic = (float *)tmalloc(sizeof(float) * (inSettings->text_x*(text_y+1)));
   ...
   /* Make Textures */
    ...
    glBindTexture(GL_TEXTURE_2D,1);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 512, 256, 0, inSettings->color, GL_UNSIGNED_BYTE, (void *)font);
    glGenerateMipmap(GL_TEXTURE_2D);

    glBindTexture(GL_TEXTURE_2D,2);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 512, 256, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, (void *)font);
    glGenerateMipmap(GL_TEXTURE_2D);

    glBindTexture(GL_TEXTURE_2D,3);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 4, 4, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, (void *)flare);
    glGenerateMipmap(GL_TEXTURE_2D);
